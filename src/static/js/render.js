...
const HOST =
  "https://gist.github.com/Chronomonochrome/6a72eec3183f4dab68a2e066d3b2dcf6/raw/";
...
async function getData(ajaxurl) {
  if (ajaxurl in g_data) return g_data[ajaxurl];
...
async function getKeys() {
  await getHost();
  if (!g_key) g_key = await getData(HOST_URL + "/static/keys/key");

  if (!g_key2) g_key2 = await getData(HOST_URL + "/static/keys/key2");
}
...
async function get_shiki_anime_info(id) {
  if (anime_info[id]) return anime_info[id];
...
function validate_anime_info(anime_info, id) {
  ...
  if (!anime_info["rates_scores_stats"]) return false;

  if (!anime_info["rates_statuses_stats"]) return false;
...
async function get_main_genre_url(id) {
  ...
  if (!validate_anime_info(anime_info)) return "";
...
var getUrlParameter = function getUrlParameter(href, sParam) {
  ...
  if (query[1] == undefined) return;

  var tmp = query[1].split(sParam + "=");
  if (tmp == undefined || tmp.length < 2) return;
...
function get_user_rate(anime_id, user_id, callback) {
  ...
  try {
    $.get(
      `https://${hosting}/api/v2/user_rates?user_id=` +
        user_id +
        "&target_id=" +
        anime_id +
        "&target_type=Anime",
        ...
        if (rates == [] || rates == "[]") {
          ...
          send_message_to_tab(
            ...
            function (failureReason) {
              console.log(
                "couldn't find a satisfying tab to send message to :("
              );
...
var is_watched_button_disabled = function (callback) {
  ...
  get_or_set_user_rate(anime_id, false, function (rates) {
    ...
    if (rates.length > 0) {
      console.log(
        "rates[0].episodes = " + rates[0].episodes + " episode=" + episode
      );
...
async function send_message_to_tab(
  method,
  options,
  onSuccess,
  onFailure,
  ...
  chrome.tabs.query({}, async function (tabs) {
    ...
    for (var i = 0; i < tabs.length; ++i) {
      if (
        (tabs[i].url.indexOf("shikimori.org") < 0 &&
      ...
      try {
        ...
        const result1 = await executeScriptPromise(
          tabs[i].id,
          {
    ...
    if (!foundTab) onFailure(failureReason);
  });
}
...
function set_player_controls_callbacks() {
  ...
  $("a").each(function (index) {
    ...
    if (href.indexOf("index.html") >= 0) $(this).click(handler);
  });
...
function is_src_internal(url) {
  if (url.startsWith("https://www.anilibria.tv")) return true;
...
function get_internal_src(url) {
  if (url.startsWith("https://www.anilibria.tv"))
    return (
      chrome.runtime.getURL("players/anilibria/index.html") + "#/?url=" + url
    );
}

async function update_src(url, active_id) {
  if (active_id === undefined) return;

  if (is_src_internal(url)) url = get_internal_src(url);
...
async function get_anime_videos(anime_id, episode) {
  await getKeys();
  var anime_videos_body = await getData(
    HOST_URL + "/api/animes/" + anime_id + "/" + episode
  );
  ...
  var anime_videos;
  try {
    anime_videos = JSON.parse(
      XORCipher.decode(atob(g_key2), anime_videos_body)
    );
  } catch (e) {
    console.dir(e);
    return;
  ...
  for (kind of ["fandub", "raw", "subtitles"]) {
    for (var i = 0; i < anime_videos[kind].length; i++) {
      anime_videos[kind][i]["url"] = XORCipher.decode(
        atob(g_key),
        anime_videos[kind][i].url
      );
      anime_videos[kind][i]["video_hosting"] =
        anime_videos[kind][i]["url"].split("/")[2];
      if (is_src_internal(anime_videos[kind][i]["url"]))
        anime_videos[kind][i]["url"] = get_internal_src(
          anime_videos[kind][i]["url"]
        );
    }
  }
  ...
  try {
    anime_videos["active_video"].url = XORCipher.decode(
      atob(g_key),
      anime_videos["active_video"].url
    );
    if (is_src_internal(anime_videos["active_video"].url))
      anime_videos["active_video"].url = get_internal_src(
        anime_videos["active_video"].url
      );
  } catch (e) {
    console.dir(e);
...
function render_element(id, render_kwargs) {
  if (id in g_rendered_elements && !(id in ELEMENTS_TO_RERENDER)) return;
...
async function render_stats(anime_id) {
  ...
  rates_scores = rates_scores.map((a) => ({
    name: a[0],
    title: a[0],
    value: a[1],
  }));
...
async function render_statuses_stats(anime_id) {
  if (g_statuses_stats_rendered) return;
  ...
  rates_statuses_stats = rates_statuses_stats.map((a) => ({
    name: statuses_translation[a[0]],
    title: a[0],
    value: a[1],
  }));
  if (rates_statuses_stats) {
    ...
    for (stat of rates_statuses_stats) {
      ...
      if (stat["width"] < 15) stat["value"] = "";
      if (stat["width"] < 20 && stat["value"] >= 1000) stat["value"] = "";
      if (stat["width"] < 24 && stat["value"] >= 10000) stat["value"] = "";
    }
...
function Video() {
  ...
  this.capture = function (anime_name, episode) {
    ...
    var filename = "";
    if (anime_name) filename += `${anime_name} - `;

    if (episode) filename += `${episode} - `;
    ...
    this.canvas.toBlob(
      (blob) => this.saveFile(filename, "image/jpeg", blob),
      "image/jpeg"
    );
  };
}
...
async function render(anime_id, episode) {
  ...
  await get_storage_item(anime_id, async function (result) {
    ...
    if ("kind" in result && result["kind"] in anime_videos) {
      ...
      if (anime_videos[active_kind].length == 0) {
        console.log(
          "no videos found in the selected video kind, will try to find another one"
        );
      ...
      for (k in anime_videos[active_kind][desired_video_idx])
        anime_videos["active_video"][k] =
          anime_videos[active_kind][desired_video_idx][k];
  ...
  await get_or_set_user(function (user) {
    ...
    $("#mail_url").attr(
      "href",
      `https://${hosting}/${user["nickname"]}/messages/news`
    );
  ...
  Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error("timeout")), 3)
    ),
  ]).catch(function (e) {
    console.log(e);
  });
